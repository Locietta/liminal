...
def make_server(port=0):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def handle_post(self):
            ...
            last_texts = [
                b.get("text", "") for b in last["content"] if b["type"] == "text"
            ]
            ...
            if any("context checkpoint compaction" in t for t in last_texts):
                ...
                self.send_text_turn(
                    "SUMMARY: user asked for cwd; ran pwd in the repo; read README.",
                    "req_compact",
                )
            ...
            first_texts = [
                b.get("text", "") for b in first["content"] if b["type"] == "text"
            ]
            if first["role"] == "user" and any(
                "[Compacted context]" in t for t in first_texts
            ):
                assert any("SUMMARY:" in t for t in first_texts), (
                    "summary missing from bridge"
                )
                assert len(body["messages"]) == 2, (
                    f"expected [bridge, prompt], got {len(body['messages'])}"
                )
                ...
                self.send_text_turn(
                    "Continuing from the compacted context.", "req_post_compact"
                )
                ...
                events = [
                    ...
                    (
                        "content_block_delta",
                        {
                            ...
                            "delta": {
                                "type": "thinking_delta",
                                "thinking": "I should check the cwd.",
                            },
                        },
                    ),
                    (
            ...
            assert last["role"] == "user", (
                f"expected user tool_result message, got {last['role']}"
            )
            assert len(last["content"]) == 2, (
                f"expected 2 tool_results, got {len(last['content'])}"
            )
            cmd_result, file_result = last["content"]
            assert (
                cmd_result["type"] == "tool_result"
            ...
            assert (
                file_result["type"] == "tool_result"
                and file_result["tool_use_id"] == "toolu_2"
            )
            assert "Liminal" in file_result["content"], (
                "read_file result missing README content"
            )
            assistant = body["messages"][-2]
            assert assistant["role"] == "assistant"
            thinking = assistant["content"][0]
            assert thinking["type"] == "thinking", (
                f"thinking not replayed first, got {thinking['type']}"
            )
            assert thinking["thinking"] == "I should check the cwd."
            assert thinking["signature"] == "sig-abc123", (
                "thinking signature not bit-exact"
            )
            self.send_text_turn(
                "The working directory is the liminal repo.", "req_final"
            )
