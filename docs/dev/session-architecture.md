...
Normal startup inspects pending markers and the bounded staging directory. It tries each relevant session lease without waiting, preserves work owned by another process, and removes abandoned staging before clearing its orphan marker. Missing-catalog rebuild and explicit repair may scan published session directories, but use validated read-only connections to read singleton rows alongside live WAL writers and never decode history payloads. Before a new catalog schema becomes valid, Liminal durably creates `catalog-rebuild-pending`; repository startup serializes and completes the rebuild before durably removing that marker. A crash or rebuild error therefore leaves an unambiguous instruction for the next opener. Catalog initialization and rebuild completion are serialized across processes, and a corrupt catalog is not silently replaced while another process may hold it open.
