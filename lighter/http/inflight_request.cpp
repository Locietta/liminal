...
constexpr int tls_version_rank(http::TlsVersion value) noexcept {
    switch (value) {
        case http::TlsVersion::TLS1_0: return 10;
        case http::TlsVersion::TLS1_1: return 11;
        case http::TlsVersion::TLS1_2: return 12;
        case http::TlsVersion::TLS1_3: return 13;
    }
...
bool bind_easy(CURL *easy, const std::shared_ptr<SharedResources> &shared, bool enable_record_cookie) noexcept {
...
long to_curl_ssl_min(http::TlsVersion value) noexcept {
    switch (value) {
        case http::TlsVersion::TLS1_0: return CURL_SSLVERSION_TLSv1_0;
        case http::TlsVersion::TLS1_1: return CURL_SSLVERSION_TLSv1_1;
        case http::TlsVersion::TLS1_2: return CURL_SSLVERSION_TLSv1_2;
        case http::TlsVersion::TLS1_3: return CURL_SSLVERSION_TLSv1_3;
    }

    return CURL_SSLVERSION_DEFAULT;
}

long to_curl_ssl_max(http::TlsVersion value) noexcept {
    switch (value) {
        case http::TlsVersion::TLS1_0:
#ifdef CURL_SSLVERSION_MAX_TLSv1_0
            return CURL_SSLVERSION_MAX_TLSv1_0;
#else
            return 0;
#endif
        case http::TlsVersion::TLS1_1:
#ifdef CURL_SSLVERSION_MAX_TLSv1_1
            return CURL_SSLVERSION_MAX_TLSv1_1;
#else
            return 0;
#endif
        case http::TlsVersion::TLS1_2:
#ifdef CURL_SSLVERSION_MAX_TLSv1_2
            return CURL_SSLVERSION_MAX_TLSv1_2;
#else
            return 0;
#endif
        case http::TlsVersion::TLS1_3:
...
bool InflightRequest::apply_method() noexcept {
    ...
    if (detail::iequals(method_name, http::method::head)) {
        return easy_setopt(*this, CURLOPT_NOBODY, 1L) && easy_setopt(*this, CURLOPT_CUSTOMREQUEST, method_name.c_str());
    }
...
bool InflightRequest::apply_body() noexcept {
    ...
    if (detail::iequals(method_name, http::method::get) || detail::iequals(method_name, http::method::head)) {
        return fail(Error::invalid_request("request body is not supported for GET or HEAD"));
    }

    return easy_setopt(*this, CURLOPT_POSTFIELDSIZE_LARGE, static_cast<curl_off_t>(body_text.size())) &&
...
bool InflightRequest::apply_redirect() noexcept {
    ...
    return easy_setopt(*this, CURLOPT_FOLLOWLOCATION, 1L) &&
           easy_setopt(*this, CURLOPT_MAXREDIRS, static_cast<long>(redirect_policy_value.max_redirects)) &&
...
bool InflightRequest::apply_tls() noexcept {
    ...
    if (!easy_setopt(*this, CURLOPT_PROTOCOLS_STR, protocols) || !easy_setopt(*this, CURLOPT_REDIR_PROTOCOLS_STR, protocols)) {
        return false;
    }
#else
    long protocols = tls_config.https_only ? CURLPROTO_HTTPS : static_cast<long>(CURLPROTO_HTTP | CURLPROTO_HTTPS);
    if (!easy_setopt(*this, CURLOPT_PROTOCOLS, protocols) || !easy_setopt(*this, CURLOPT_REDIR_PROTOCOLS, protocols)) {
        return false;
    }
#endif

    if (!easy_setopt(*this, CURLOPT_SSL_VERIFYPEER, tls_config.danger_accept_invalid_certs ? 0L : 1L) ||
        !easy_setopt(*this, CURLOPT_SSL_VERIFYHOST, tls_config.danger_accept_invalid_hostnames ? 0L : 2L)) {
        return false;
    }
...
bool InflightRequest::prepare() noexcept {
    ...
    return apply_url() && apply_method() && apply_body() && apply_headers() && apply_cookies() && apply_user_agent() && apply_redirect() &&
...
Outcome<Response, Error, Cancellation> InflightRequest::finish() noexcept {
    ...
    if (auto err = curl::getinfo(easy.get(), CURLINFO_EFFECTIVE_URL, &effective); curl::ok(err) && effective != nullptr) {
        out.url = effective;
    } else {
        out.url = final_url;
    }
