...
struct TerminalInputDecoder {
    void feed(std::string_view bytes, FunctionRef<void(TerminalEvent)> emit);

    /// Resolves an incomplete escape prefix after the terminal's escape-key
    /// disambiguation timeout expires.
    void flush_escape(FunctionRef<void(TerminalEvent)> emit);

    bool escape_pending() const noexcept;

private:
    void parse(bool flush_escape, FunctionRef<void(TerminalEvent)> emit);
